...
@objc(PDFDocumentManager) public class PDFDocumentManager: NSObject {
    ...
    @objc func getPageInfoForPageAtIndex(_ reference: NSNumber, pageIndex: Int, onSuccess: @escaping RCTPromiseResolveBlock, onError: @escaping RCTPromiseRejectBlock) -> Void {
        guard let document = getDocument(reference) else {
            onError("getPageInfoForPageAtIndex", "Document is nil", nil)
            return
        }
        
        guard let pageInfo = document.pageInfoForPage(at: PageIndex(pageIndex)) else {
            onError("getPageInfoForPageAtIndex", "Could not retrieve page info", nil)
            return
        }
