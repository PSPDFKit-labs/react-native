...
- (void)pspdfView:(RCTPSPDFKitView *)view didTapCustomTextSelectionMenuItemWithId:(NSString *)itemId {
  if (_eventEmitter) {
    facebook::react::NutrientViewEventEmitter::OnCustomTextSelectionContextualMenuItemTapped payload{
      itemId ? std::string([itemId UTF8String]) : std::string("")
    };
    _eventEmitter->onCustomTextSelectionContextualMenuItemTapped(payload);
  }
}
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  if (newProps != nullptr) {
      ...
      // Handle text selection contextual menu via JSON string (presence aware)
      if (!newProps->textSelectionContextualMenuJSONString.empty()) {
        NSString *menuJSON = RCTNSStringFromString(newProps->textSelectionContextualMenuJSONString);
        NSDictionary *textMenuDict = [NutrientFabricUtils dictionaryFromJSONString:menuJSON];
        if (textMenuDict) {
          [NutrientPropsAnnotationsHelper applyTextSelectionContextualMenuFromJSON:textMenuDict toView:_view];
        }
      }
  }
}
