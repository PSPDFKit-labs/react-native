...
- (UIMenu *)pdfViewController:(PSPDFViewController *)sender menuForText:(PSPDFGlyphSequence *)glyphs onPageView:(PSPDFPageView *)pageView appearance:(PSPDFEditMenuAppearance)appearance suggestedMenu:(UIMenu *)suggestedMenu {
    
    NSDictionary *textButtons = [_sessionStorage getTextSelectionContextualMenuItems];
    NSArray<NSString *> *stockActions = textButtons[@"stockActions"];
    
    if (textButtons[@"appearance"] == nil ||
        appearance == [RCTConvert PSPDFEditMenuAppearance:textButtons[@"appearance"]]) {
        
        NSArray *customItems = textButtons[@"buttons"];
        if ([textButtons count] == 0 || !customItems) {
            return suggestedMenu;
        }
        
        BOOL retainSuggested = (textButtons[@"retainSuggestedMenuItems"] == nil ||
                                [textButtons[@"retainSuggestedMenuItems"] boolValue] == YES);
        BOOL hasStockActions = (stockActions && [stockActions isKindOfClass:[NSArray class]] && stockActions.count > 0);
        
        // Start from the full suggested menu by default.
        UIMenu *baseMenu = suggestedMenu;
        
        // When retainSuggested is false and explicit stockActions are provided, filter the
        // suggested menu down to only those stock actions. This mirrors the Android behavior
        // where stock strings only matter when retainSuggestedMenuItems is false.
        if (!retainSuggested && hasStockActions) {
            NSSet<NSString *> *allowed = [NSSet setWithArray:stockActions];
            baseMenu = [self pspdf_filteredMenuFromMenu:suggestedMenu allowedKeys:allowed];
        }
        ...
        NSArray *children;
        if (retainSuggested || hasStockActions) {
            // Either keeping all suggested items (retainSuggested == YES) or using the
            // filtered stock-only menu (retainSuggested == NO && hasStockActions).
            if (textButtons[@"position"] != nil && [textButtons[@"position"] isEqualToString:@"start"]) {
                children = [customItems arrayByAddingObjectsFromArray:baseMenu.children];
            } else {
                children = [baseMenu.children arrayByAddingObjectsFromArray:customItems];
            }
        } else {
            // retainSuggested == NO and no stockActions provided: only show custom items.
            children = customItems;
        }
        
        UIMenu *newMenu = [baseMenu menuByReplacingChildren:children];
        return newMenu;
    } else {
        return suggestedMenu;
    }
...
// Helper to filter a UIMenu so that only actions whose identifier contains one of
// the allowed string keys (case-insensitive) are kept. Submenus are processed
// recursively.
- (UIMenu *)pspdf_filteredMenuFromMenu:(UIMenu *)menu allowedKeys:(NSSet<NSString *> *)allowedKeys {
    NSMutableArray<UIMenuElement *> *filteredChildren = [NSMutableArray array];
    for (UIMenuElement *element in menu.children) {
        if ([element isKindOfClass:UIAction.class]) {
            UIAction *action = (UIAction *)element;
            NSString *rawIdentifier = action.identifier;
            if (rawIdentifier != nil && allowedKeys.count > 0) {
                NSString *identifierLowercased = [rawIdentifier lowercaseString];
                BOOL isAllowed = NO;
                for (NSString *allowedKey in allowedKeys) {
                    if (![allowedKey isKindOfClass:NSString.class]) {
                        continue;
                    }
                    NSString *keyLowercased = [allowedKey lowercaseString];
                    if (keyLowercased.length > 0 && [identifierLowercased containsString:keyLowercased]) {
                        isAllowed = YES;
                        break;
                    }
                }
                if (isAllowed) {
                    [filteredChildren addObject:action];
                }
            }
        } else if ([element isKindOfClass:UIMenu.class]) {
            UIMenu *filteredSubmenu = [self pspdf_filteredMenuFromMenu:(UIMenu *)element allowedKeys:allowedKeys];
...
// MARK: - Customize the Text Selection Contextual Menu

- (void)setTextSelectionContextualMenuItems:(NSDictionary *)items {
    
    NSArray *buttonItems = items[@"buttons"];
    
    NSMutableArray *customButtons = [NSMutableArray array];
    NSMutableArray<NSString *> *stockActions = [NSMutableArray array];
    for (id rawItem in buttonItems) {
        if ([rawItem isKindOfClass:NSString.class]) {
            // Stock item identifier; keep separately so we can filter the suggestedMenu later.
            ...
            NSDictionary *item = (NSDictionary *)rawItem;
            UIAction *menuAction = [UIAction actionWithTitle:item[@"title"] image:nil identifier:item[@"id"] handler:^(__kindof UIAction * _Nonnull action) {
              [self handleCustomTextSelectionContextualMenuItemEvent:action];
            }];
            ...
            [customButtons addObject:menuAction];
        }
    }
    
    NSMutableDictionary *updatedItems = [NSMutableDictionary dictionaryWithDictionary:items];
    [updatedItems setObject:customButtons forKey:@"buttons"];
    if (stockActions.count > 0) {
        [updatedItems setObject:stockActions forKey:@"stockActions"];
    } else {
        [updatedItems removeObjectForKey:@"stockActions"];
    ...
    [_sessionStorage setTextSelectionContextualMenuItems:updatedItems];
}

// MARK: - Text Selection Contextual Menu Event Handling

- (void)handleCustomTextSelectionContextualMenuItemEvent:(id)sender {
    UIAction *actionButton = (UIAction *)sender;
    if (self.onCustomTextSelectionContextualMenuItemTapped) {
        NSString *customId = actionButton.identifier;
        if (customId != nil) {
            self.onCustomTextSelectionContextualMenuItemTapped(@{ @"id" : customId });
        }
    }
    NSString *customId = actionButton.identifier;
    if (customId && [self.delegate respondsToSelector:@selector(pspdfView:didTapCustomTextSelectionMenuItemWithId:)]) {
        [self.delegate pspdfView:self didTapCustomTextSelectionMenuItemWithId:customId];
    }
}
